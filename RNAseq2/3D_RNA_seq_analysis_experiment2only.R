...
contrast <- c('HEB_124_17-Barke','HEB_124_52-Barke','HEB_124_52-HEB_124_17')
...
design <- condition2design(condition = metatable_new$Tissue,
...
idx <- grep('Tissue',colnames(design))
  design.idx <- colnames(design)
  design.idx <- substr(design.idx,nchar('Tissue')+1,nchar(design.idx))
...
HEB52_HEB17 <- subset(DE_genes,DE_genes$contrast=="HEB_124_52-HEB_124_17",drop=TRUE)
HEB52_Barke <- subset(DE_genes,DE_genes$contrast=="HEB_124_52-Barke",drop=TRUE)
HEB17_Barke <- subset(DE_genes,DE_genes$contrast=="HEB_124_17-Barke",drop=TRUE)
...
length(HEB52_Barke_HEB17_Barke)
length(HEB52_HEB17_HEB17_Barke)
...
#Just experiment 2 results, no experiment 1 

#NOw HEB124_52 - HEB_124_17 subset in Hid124_52-Barke comparison
col0='black'
col1='#E69F00'
col2='#56B4E9'
adj_p52<-genes_3D_stat_control$DE.pval[,3]
lgfc52<-genes_3D_stat_control$DE.lfc[,3]
chromosome = "chr3H"
...
HEB52_HEB17 <- DE_genes_control %>% filter(contrast == "HEB_124_52-HEB_124_17")
HEB52_Barke <- DE_genes_control %>% filter(contrast == "HEB_124_52-Barke")
...
gene_name <- "BaRT2v18chr1HG043390"
...
for (i in 1:length(genotypes)){
 genotype_names[i] <- str_replace(genotypes[i], "HEB_", "")
 i = i + 1
}

genotype_names <- unlist(genotype_names)
for_plot <- data.frame(CPM=test2,genotype=genotype_names, genotype_only= genotype_names)
...
png(filename = paste0(figure.folder, "/", gene_name, "_jitter.png"),width = 600, height = 600, units = "px", pointsize = 12,bg = "white")
	print({
g <- ggplot(for_plot, aes(x=genotype_only, y=CPM, colour=genotype_only)) +
...
DE_experiment_17_52 <- genes_3D_stat$DE.stat %>% filter(contrast == "HEB_124_52-HEB_124_17")


sig <- DE_experiment_17_52 %>% filter(adj.pval <= 0.01) %>% filter(log2FC >= 1 | log2FC <= -1) %>% pull(target) %>% unique()


enriched_GOs <- functionalEnrichment(genes_all, genes_sel=sig, gene2term, terms)

#Try with subset:

sig_124_52_barke <- genes_3D_stat$DE.stat %>% filter(contrast == "HEB_124_52-Barke") %>% filter(adj.pval <= 0.01) %>% filter(log2FC >= 1 | log2FC <= -1) %>% pull(target) %>% unique()

sig_subset <- DE_experiment_17_52 %>% filter(adj.pval <= 0.01) %>% filter(log2FC >= 1 | log2FC <= -1) %>% filter(target %in% sig_124_52_barke) %>% pull(target) %>% unique()

enriched_GOs <- functionalEnrichment(genes_all, genes_sel=sig_subset, gene2term, terms)



#Nothing! Try with both contrasts?


DE_experiment_both <- genes_3D_stat$DE.stat %>% filter(contrast == "HEB_124_52-HEB_124_17" | contrast == "HEB_124_52-Barke")
sig <- DE_experiment_both %>% filter(adj.pval <= 0.01) %>% filter(log2FC >= 1 | log2FC <= -1) %>% pull(target) %>% unique() #All 87 genes

enriched_GOs <- functionalEnrichment(genes_all, genes_sel=sig, gene2term, terms)
#Result!!!

write.csv(enriched_GOs, paste0(result.folder, "/", "experiment2only_bothcontrasts.csv"))

#NOw try with genes only in just 124_52-Barke


DE_experiment_52_Barke <- genes_3D_stat$DE.stat %>% filter(contrast == "HEB_124_52-Barke")

DE_experiment_17_52 <- genes_3D_stat$DE.stat %>% filter(contrast == "HEB_124_52-HEB_124_17")
sig_17_52 <- DE_experiment_17_52 %>% filter(adj.pval <= 0.01) %>% filter(log2FC >= 1 | log2FC <= -1) %>% pull(target) %>% unique()



sig <- DE_experiment_52_Barke %>% filter(adj.pval <= 0.01) %>% filter(log2FC >= 1 | log2FC <= -1) %>% filter(!(target %in% sig_17_52)) %>% pull(target) %>% unique() 
...
png(filename = paste0(figure.folder, "/","experiment2only_bothcontrasts_bubble.png"),width = 600, height = 600, units = "px", pointsize = 12,bg = "white",quality = 100)
	print({
g <- ggplot(enriched_GOs, aes(x=sel, y=-log10(P), size=enrich)) + theme_bw() + geom_point(alpha=0.7,colour="#0072B2") + scale_size(range = c(2, 15),name="Enriched") + geom_label_repel(data=enriched_GOs,aes(x=sel,y=-log10(P),label=description),min.segment.length = 0, size = 3, show.legend=FALSE, fontface = 'bold',segment.color = 'black',force=8,nudge_x=2,nudge_y=2) + labs(x="Number of selected genes with GO terms",y="-log10(p)",title = "Significantly Enriched GO terms") + theme(axis.text.x=element_text(size=20),axis.text.y=element_text(size=20),
...
enriched_GOs <- functionalEnrichment(genes_all, genes_sel=sig, gene2term, terms)
#NOthing :(
#write.csv(enriched_GOs, paste0(result.folder, "/", "experiment2only_bothcontrasts_GO_slims.csv"))
...
#Now pull out genes that are expressed in 3H locus
expressed_genes <- unique(rownames(genes_3D_stat$DE.pval))

locus3H_genes_expressed <- read_tsv(gene_annotation) %>% filter(Chromosome == "chr3H") %>% filter(Start >= 33181340) %>% filter(End <= 36970860) %>% select(`BaRTv2 gene`, Chromosome, Start, End, `Pannzer annotation`, `Coding potentiality`) %>% filter(`BaRTv2 gene` %in% expressed_genes)
#25 genes expressed
write.csv(locus3H_genes_expressed, paste0(result.folder, "/", "all_3H_genes_expressed.csv"))

#Write out all genes, but with expression information:
locus3H_genes_expressed <- locus3H_genes_expressed %>% column_to_rownames( var = "BaRTv2 gene")
expression_info <- list()
for (i in 1:nrow(locus3H_genes)){
	
	if (as.character(locus3H_genes[i,1]) %in% rownames(locus3H_genes_expressed)) {
      expression_info[i] <- "Expressed"
	} else {
      expression_info[i] <- "Not expressed"
	}
}

expression_info <- unlist(expression_info)

locus3H_genes <- locus3H_genes %>% mutate(Expression = expression_info)


write.csv(locus3H_genes, paste0(result.folder, "/", "all_3H_genes.csv"))


#Subset of 34 that are present in 124_52 - 124_17, 124_52 - Barke and absent from 124_17 - Barke
DE_genest <- as_tibble(DE_genes) #Convert to tibble so all things work ok

possible_contrasts <- DE_genest %>% select(contrast) %>% unique()


c52_17 <- DE_genest %>% filter(contrast == "HEB_124_52-HEB_124_17")

c52_Barke <- DE_genest %>% filter(contrast == "HEB_124_52-Barke")

c17_Barke <- DE_genest %>% filter(contrast == "HEB_124_17-Barke")#0


#Filter for genes present in c52_17 and c52_Barke and not in c17_Barke
interest <- c52_17 %>% filter(target %in% c52_Barke$target)

interest_all <- inner_join(interest, BaRTv2, by='target')

write.csv(interest_all,paste0(result.folder,"/candidate_gene_complete_info34_experiment2only.csv"))
#Include information

#How many genes expressed in dataset?

length(unique(rownames(genes_3D_stat$DE.pval))) #18,577

#NOw write out all 80 DE genes:
c52_17 <- DE_genest %>% filter(contrast == "HEB_124_52-HEB_124_17")
c52_Barke <- DE_genest %>% filter(contrast == "HEB_124_52-Barke") %>% filter(!(target %in% c52_17$target))
both <- rbind(c52_17,c52_Barke)#All 52_17 and the extra 52_Barke

both_annotated <- inner_join(both, BaRTv2, by='target')

write.csv(both_annotated,paste0(result.folder,"/candidate_genes_allDE_experiment2only.csv"))



#DTU analysis - find DTU transcripts for 3H locus

DTUc52_17 <- DTU_trans %>% filter(contrast == "HEB_124_52-HEB_124_17")

DTUc52_Barke <- DTU_trans %>% filter(contrast == "HEB_124_52-Barke")


#Filter for transcripts present in c52_17 and c52_Barke and not in c17_Barke
DTUinterest <- DTUc52_17 %>% filter(target %in% DTUc52_Barke$target)
...
DASc52_17 <- DAS_genes %>% filter(contrast == "HEB_124_52-HEB_124_17")

DASc52_Barke <- DAS_genes %>% filter(contrast == "HEB_124_52-Barke")


DASinterest <- DASc52_17 %>% filter(target %in% DASc52_Barke$target)
...
write.csv(DASinterest,paste0(result.folder,"/DAS_52-17_subsetexperiment2only.csv"))